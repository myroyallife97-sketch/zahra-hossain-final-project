...
// default constructor -- if we create a location with no values, these are the automatic placeholder/default settings
Location::Location() {
    id = 0;
    name = "Unknown";
...
//lets us create a location and immediately
// assign values like the id, name, and description
Location::Location(int locationId, const string& locationName, const string& locationDescription) {
    id = locationId;
    name = locationName;
    description = locationDescription;

    // every location starts off as unvisited at the beginning of the game
    visited = false;
}

// setter function for updating/changing the id
void Location::setId(int newId) {
    id = newId;
}

// setter function for updating/changing the location name
void Location::setName(const string& newName) {
    name = newName;
}

// setter function for updating/changing the location description
void Location::setDescription(const string& newDescription) {
    description = newDescription;
}

// updates whether the player has visited this location yet or not
void Location::setVisited(bool status) {
    visited = status;
}

// getter function that returns the location id
int Location::getId() const {
    return id;
}

// getter function that returns the location name
string Location::getName() const {
    return name;
}

// getter function that returns the location description
string Location::getDescription() const {
    return description;
}

// getter function that returns whether the location has been visited
bool Location::getVisited() const {
    return visited;
}

// prints out the location name + description for the player
void Location::displayLocation() const {
    cout << name << endl;
    cout << description << endl;
}

// creates/stores all the Shepard Hall locations inside a vector
// this makes it easier to organize the map instead of manually storing everything separately
vector<Location> createShepardHallLocations() {

    // vector that will hold all the location objects
    vector<Location> locations;

    // each push_back creates a new Location object and adds it to the vector along w short descriptions of each location
    ...
    // returns the completed vector of all the Shepard Hall locations
    return locations;
}

// portal transition locations -- these are the spots between Shepard Hall and the fantasy world
...
// Kingsport locations -these store the main fantasy city section before the player reaches the Keep
...
// Crimson Keep and scholar district locations -- this is the castle approach + search for answers section
...
// ending locations -- these are the final reveal + return home sections
...
// combines all location vectors into one bigger vector so the whole map can exist together
vector<Location> createAllLocations() {
    ...
    // these loops copy each smaller section into the full map vector
